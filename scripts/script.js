...
if (menuIcon && closeIcon && menuOverlay) {
...
async function loadBlogPosts(container) {
  try {
    const response = await fetch("../blog/blog.json"); // Исправленный путь
    if (!response.ok) {
      throw new Error("Ошибка загрузки данных: " + response.statusText);
    }
...
// Загружаем посты во всплывающее меню
const menuPostsContainer = document.querySelector(".menu-overlay .blog-posts");
if (menuPostsContainer) {
  loadBlogPosts(menuPostsContainer);
}

// Загружаем посты на странице blog.html
const blogPageContainer = document.querySelector(".blog-page .blog-posts");
if (blogPageContainer) {
  loadBlogPosts(blogPageContainer);
}
